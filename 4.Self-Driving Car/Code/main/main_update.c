...
//virtual beacon
esp_err_t beacon_get_handler(httpd_req_t *req)
{
    if (bcon == false) {
        bcon = true;
        const char* resp_str = "BEACON DETECTED";
        httpd_resp_set_hdr(req, "access-control-allow-origin", "*");
        httpd_resp_send(req, resp_str, strlen(resp_str));
    } else {
        bcon = false;
        const char* resp_str = "NO BEACON DETECTED";
        httpd_resp_set_hdr(req, "access-control-allow-origin", "*");
        httpd_resp_send(req, resp_str, strlen(resp_str));
    }
    return ESP_OK;
...
httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(server, &vbcon);
...
void app_main()
{
  ...
  while(true) {
    ...
    if (nvtimer != vtimer) {
      ...
      //printf("Calling Beacon \n");
      //beacon[0] = updateBeacon();  // IR RX: check for value and update beacon
     ...
     // IR RX: if there is data, update the variable 'beacon'
      ...
      switch (state) {
        case 'I':
          ...
          break;
        case 'D':
          if (prevState[0] != 'D') {  // First time in Driving state
            forward(0.0,0.0);  // Turn on the motors and move forward
            nextState[0] = 'D';
          } else {  // Already in Driving state for at least 1 cycle
            if (sideError != 0 && frontError > 0) nextState[0] = 'A';
            else if (frontError <= 0 && beacon[0] != '9') nextState[0] = 'T';
            else if (frontError <= 0 && beacon[0] == '9') nextState[0] = 'W';
          }
          break;
        case 'W':
        ...
        case 'T':
          if (prevState[0] != 'T') {  // First time in Turning state
            ...
            if (distSide > distSidePrev && frontError > 0) 
                nextState[0] = 'D';
            else 
                nextState[0] = 'T';
          }
