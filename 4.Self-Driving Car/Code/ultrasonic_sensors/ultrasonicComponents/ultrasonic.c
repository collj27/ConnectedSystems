...
void ultrasonic_init2(const ultrasonic_sensor_t2 *dev)
{
    gpio_set_direction(dev->trigger_pin2, GPIO_MODE_OUTPUT);
    gpio_set_direction(dev->echo_pin2, GPIO_MODE_INPUT);

    gpio_set_level(dev->trigger_pin2, 0);
}
...
esp_err_t ultrasonic_measure_cm2(const ultrasonic_sensor_t2 *dev, uint32_t max_distance, uint32_t *distance)
{
    if (!distance)
        return ESP_ERR_INVALID_ARG;
    ...
    gpio_set_level(dev->trigger_pin2, 0);
    ets_delay_us(TRIGGER_LOW_DELAY);
    gpio_set_level(dev->trigger_pin2, 1);
    ets_delay_us(TRIGGER_HIGH_DELAY);
    gpio_set_level(dev->trigger_pin2, 0);

    // Previous ping isn't ended
    if (gpio_get_level(dev->echo_pin2))
        RETURN_CRTCAL(mux, ESP_ERR_ULTRASONIC_PING);

    // Wait for echo
    uint32_t start = get_time_us();
    while (!gpio_get_level(dev->echo_pin2))
    {
        if (timeout_expired(start, PING_TIMEOUT))
            RETURN_CRTCAL(mux, ESP_ERR_ULTRASONIC_PING_TIMEOUT);
    }
